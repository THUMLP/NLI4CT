...
from models import BertModel_1024, model_tf_bi
...
class LpaProcessor(object):
    def get_examples(self, data_dir, dataset=None, fold=0):
        if 'test' in dataset:
            ...
            logger.info('Get examples from: {}_{}.json'.format(dataset, fold))
            return self._create_examples(self._read_tsv("new_CV/{}_{}.json".format(dataset, fold)))
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, phase):
    ...
    for (ex_index, example) in enumerate(tqdm(examples, desc="convert to features")):
        ...
        for i in range(primary_num):
            ...
            if i == 0:
                section_ids += [-2]*evidence_length[i]
            else:
        ...
        if len(evidence_strs) > 1:
            for i in range(len(trunc_all_evidence_tokens)-primary_num):
                ...
                if i == 0:
                    section_ids += [-2]*evidence_length[i+primary_num]
                else:
                    section_ids += [i+primary_num-2]*evidence_length[i+primary_num]
        tokens += ['[SEP]']
        section_ids += [-2]
...
def run_train(device, processor, tokenizer,model, writer, phase="train"):
    ...
    for ep in trange(args.num_train_epochs, desc="Training"):
        ...
        for step, batch in enumerate(tqdm(tr_dataloader)):
            ...
            prob, evidence_logits, att = model(inputs)

            actual_prob = torch.exp(prob)
            all_label.append(int(label_ids))
            all_pred.append(int(torch.argmax(actual_prob, dim=1)))
            loss = F.nll_loss(prob, label_ids.view(-1))
...
def main():
    ...
    bert = BertModel_1024.from_pretrained(args.bert_model)
    bert.update_position_embedding(init_type=args.init_type)
    # bert.config.gradient_checkpointing = True
    model = model_tf_bi(bert,os.path.join(args.bert_model,'config.json'),num_layers=args.tf_num_layers)
    if args.load_dir:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default='outputs_biolinkbert_multitask_sent_transformer_1_1024_partrandom_fold_0')
    ...
    parser.add_argument("--init_type", default=1, type=int)
