...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_rnn1 = nn.Linear(config.rnn_hidden * 2, config.hidden_size)

        # Initializing a BERT bert-base-uncased style configuration
        self.configuration = BertConfig()
        self.configuration.hidden_size = 1024
        self.configuration.num_attention_heads = 16
        self.configuration.num_hidden_layers = 2
        self.block = BertModel(self.configuration)
    ...
    def forward(self, x):
        ...
        sentlevel_emb = self.block(inputs_embeds=sentlevel_emb).last_hidden_state
        # print(sentlevel_emb.shape)
        # input()

        # token级别表征融合 尝试2 lstm last
        ...
        for j in range(len(span_evi)):
            ...
            # statement_evi_emb_res = self.attention1(statement_evi_emb)
            # X = self.attention1(statement_evi_emb)
            # statement_evi_emb_res = statement_evi_emb + X

            statement_evi_emb_res_out, h_0_token = self.lstm1(statement_evi_emb)
            statement_evi_emb_res_last = h_0_token[0]
            # print("statement_evi_emb_res_last ", statement_evi_emb_res_last.shape)
            statement_evi_emb_res_last = statement_evi_emb_res_last.reshape(1, -1)
            # print("statement_evi_emb_res_last ", statement_evi_emb_res_last.shape)
            statement_evi_emb_res_last = self.fc_rnn1(statement_evi_emb_res_last)
            # print("statement_evi_emb_res_last ", statement_evi_emb_res_last.shape)
            statement_evi_emb_res_last = statement_evi_emb_res_last.reshape(1, 1, -1)
            # print("statement_evi_emb_res_last ", statement_evi_emb_res_last.shape)
            all_state_evi_emb = torch.cat((all_state_evi_emb, statement_evi_emb_res_last), 1)
            # input()
